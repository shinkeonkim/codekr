...
| 강제 탈퇴 | **같은 경로** | 두 벌로 두면 한쪽만 고치는 일이 생긴다 |
...
| 기본값 | **끔** | 열람 기록은 조회자의 프라이버시를 건드린다. 켜져 있는 것이 기본이면 "동의 없이 추적" 이 기본이 된다 |
| 쓰기 폭주 | **꺼져 있으면 아예 기록하지 않는다** | 대부분의 조회에 쓰기가 없다. 설정 하나로 해결된다 |
...
이것은 **남이 내 것을 본 것**이다 — 뜻이 다르면 끄고 켜는 단위도 달라야 한다.
