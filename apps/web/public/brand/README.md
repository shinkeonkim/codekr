...
| `banner-submissions-3.webp` | `banner/submissions-03.png` | 전체 제출 상단 슬라이드쇼 3장 |
...
**그림 속 가짜 UI 는 잘라내지 않았다.** 첫 묶음은 그림 속 버튼("TRY AGAIN",
"로그인 하기")을 잘라 냈지만, 배너는 목록 위에 까는 **장식 띠**여서 그림 속 UI 를 조작으로
착각할 자리가 아니다 — 진짜 버튼은 배너 바깥에 따로 있다. 어드민 배너의 숫자
(12,543 · 62.4%)도 실제 값이 아니다. **진짜 지표를 그 자리에 넣게 되면 그림을 뺀다.**

**배경 제거도 하지 않았다.** 배너는 배경 자체가 그림이라 걷어 낼 것이 없다.
`state-login.webp` 만 원본이 이미 투명이라 **투명 여백만 잘라 냈다**(1536×1024 →
1154×1014 → 460×404). 첫 묶음의 flood fill 은 이번에 쓸 일이 없었다.

**배너는 폭 1600 이다.** 본문 폭이 `max-w-6xl`(1152px)이라 그 1.4배다.

## 슬라이드 셋만 위쪽을 잘라 냈다 (#518)

원본 비율(1983×793 = 2.5:1)을 그대로 쓰니 본문 폭에서 **460px** 를 차지했다. 목록을
보러 온 화면인데 첫 화면이 배너로 다 찼다. **위쪽 1/3 을 잘라 3.75:1 로 만들었다**
(793 → 529 → 1600×427, 화면에서 약 300px). 위쪽은 전부 빈 하늘이라 고양이도 소품도
잃지 않았다.

**어드민 배너는 자르지 않았다.** 위쪽에 "관리의 시작," 이 박혀 있어 같은 만큼 자르면
그 글자가 잘린다. 그래서 `BrandBanner` 는 **이름마다 크기를 따로 든다** — 상수 하나로
두면 비율이 갈린 순간 그림이 눌린다.

슬라이드쇼는 **첫 장의 비율로 높이를 잡는다.** 비율이 다른 그림을 한 슬라이드쇼에
섞으면 `object-cover` 가 잘라 낸다.
