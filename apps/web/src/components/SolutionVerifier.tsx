...
import { STATUS_LABELS, VERDICT_LABELS, formatMemory, verdictTone } from "@/lib/labels";
import type { ProblemSolution, ProblemVerification, Runtime } from "@/lib/types";
...
export function SolutionVerifier({ problemId, solution, verification, onChange }: Props) {
  ...
  const verify = async () => {
    ...
    setError(null);
    try {
      setLatest(await api.verifyProblem(problemId));
    } catch (caught) {
