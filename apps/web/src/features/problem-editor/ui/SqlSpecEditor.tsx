    ...
    <div className="space-y-4">
      <div>
        <p className="mt-1 text-xs text-ink-muted">
          문제마다 새 PostgreSQL 이 뜨고, 제출 쿼리는 기본적으로 읽기 전용
          권한으로 실행됩니다.
        </p>
      </div>
      ...
      <Field label="스키마와 시드 데이터">
        <Textarea
          ...
          placeholder={
            "CREATE TABLE members (id int, city text);\nINSERT INTO members VALUES (1, '서울');"
          }
          required
        />
      </Field>
      ...
      <p className="text-xs text-ink-muted">
        채점할 때 이 쿼리를 먼저 돌려 기대 결과를 만듭니다. 시드를 고치면 기대
        결과도 따라갑니다.
      </p>
      ...
      <p className="text-xs text-ink-muted">
        ...
        비워 두면 제출 쿼리의 <b>결과 집합</b>을 견줍니다. 채우면 정답
        스크립트를 돌린 DB 와 제출을 돌린 DB 에서 각각 이 쿼리를 돌려{" "}
        <b>끝난 뒤의 상태</b>를 견줍니다 —<code>INSERT</code>·
        <code>UPDATE</code>·<code>CREATE TABLE</code> 문제에 필요합니다.
      </p>
        ...
        label={
          <>
            ...
            <span className="block font-normal text-ink-muted">
              ...
              켜면 제출 롤에 쓰기 권한을 줍니다. 위의 상태를 읽는 쿼리가 있어야
              켤 수 있습니다 — 없으면 채점이 조용히 결과 집합 비교로 돌아가 아무
              답이나 통과합니다.
            </span>
          </>
        }
      ...
      <CheckboxField
        ...
        label={
          <>
            ...
            <span className="block font-normal text-ink-muted">
              정렬이 문제의 일부라면 끄세요. 켠 채로 두는 것이 기본입니다 —
              문제가 정렬을 요구하지 않는데 순서를 비교하면 맞는 답이 틀린
              것으로 나옵니다.
            </span>
          </>
        }
