...
export function SolutionVerifier({
  ...
  const [latest, setLatest] = useState<ProblemVerification | null>(
    verification,
  );
  ...
  const update = (patch: Partial<ProblemSolution>) =>
    onChange({
      runtimeId:
        patch.runtimeId ?? solution?.runtimeId ?? runtimes[0]?.id ?? "",
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div>
          <p className="mt-0.5 text-xs text-ink-muted">
            등록하면 공개 전에 전체 테스트케이스를 검증할 수 있습니다.
            사용자에게는 노출되지 않습니다.
          </p>
        </div>
        <Select
    ...
    <div className="space-y-3 rounded-lg border border-border p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {finished && verification.verdict ? (
          <Badge tone={verdictTone(verification.verdict)}>
            {VERDICT_LABELS[verification.verdict]} · {verification.passedCount}/
      ...
      {finished && verification.verdict !== "ACCEPTED" ? (
        <p className="text-xs text-warn">
          정답 코드가 통과하지 못했습니다. 기대 출력, 실행 환경, 시간·메모리
          제한을 확인하세요.
        </p>
      ) : null}
    </div>
