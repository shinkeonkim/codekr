    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Field label="난이도">
        ...
        {values.difficulty === null ? (
          <Select
            ...
            onChange={(event) =>
              update("difficultyState", event.target.value as DifficultyState)
            }
          >
            <option value="UNRATED">미평가 — 나중에 난이도를 매긴다</option>
            <option value="NO_RATE">
              평가 안 함 — 영영 점수를 주지 않는다
            </option>
          </Select>
        ) : null}
      </Field>
      <Field label="시간 제한 (ms)">
        <Input
          ...
          onChange={(event) =>
            update("timeLimitMs", Number(event.target.value))
          }
        />
      </Field>
      <Field label="메모리 제한 (MB)">
        <Input
          ...
          onChange={(event) =>
            update("memoryLimitMb", Number(event.target.value))
          }
        />
      </Field>
      <Field label="출력 비교">
        <Select
          ...
          onChange={(event) =>
            update("outputComparison", event.target.value as OutputComparison)
          }
        >
      ...
      {values.outputComparison === "FLOAT" ? (
        <Field label="허용 오차">
          <Input
            ...
            onChange={(event) =>
              update("floatEpsilon", Number(event.target.value))
            }
          />
          {/* 오차를 크게 잡으면 틀린 답이 통과한다 — 채점을 끄는 것과 같다. */}
          <p className="text-xs text-ink-muted">
            절대·상대 오차 중 하나만 만족해도 맞은 답으로 봅니다. 보통 0.000001
            을 씁니다.
          </p>
        </Field>
      ) : null}
