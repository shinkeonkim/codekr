...
abstract class IntegrationTestBase {
    ...
    fun truncateAll() {
        ...
        */
        ...
        jdbcClient.sql(
            "DELETE FROM badges WHERE code NOT LIKE 'CATEGORY_10_%' AND code NOT IN " +
                "('FIRST_ACCEPT','STREAK_7','STREAK_30','FIRST_SOLVER','CONTEST_FIRST','CONTEST_5')",
        ).update()
        // 규칙도 마이그레이션이 넣은 것이다 (#202) — 시험이 넣은 것만 걷고 스위치를 되돌린다.
        jdbcClient.sql(
            "DELETE FROM badge_rules WHERE rule_key NOT IN " +
                "('FIRST_ACCEPT','FIRST_SOLVER','CATEGORY_10','STREAK_7','STREAK_30'," +
                "'CONTEST_FIRST','CONTEST_5')",
        ).update()
