...
data class ProblemUpsertRequest(
    ...
    /** MongoDB 유형일 때만 쓴다 (#527). 다른 유형에 실려 오면 서비스가 거부한다. */
    @field:Valid
    val mongoSpec: MongoSpecRequest? = null,

    /** 퀴즈 유형일 때만 쓴다 (#650). 다른 유형에 실려 오면 서비스가 거부한다. */
    @field:Valid
    val quizSpec: QuizSpecRequest? = null,

    /** 정규식 유형일 때만 쓴다 (#653). 다른 유형에 실려 오면 서비스가 거부한다. */
    @field:Valid
    val regexSpec: RegexSpecRequest? = null,

    /** Git 유형일 때만 쓴다 (#654). 다른 유형에 실려 오면 서비스가 거부한다. */
    @field:Valid
