...
class RankingService(private val rankingRepository: RankingRepository) {
     ...
     * 순위표 한 쪽.
     *
     * [affiliationId] 를 주면 **그 소속 사람들만** 본다 (#399). 모집단을 좁히는 것이라
     * 등수는 그 안에서 1위부터 다시 매겨진다 — "우리 학교에서 3등" 이 이 기능의 이유다.
     */
    fun page(
        ...
        val entries = rankingRepository.findPage(metric, period, size, page * size, affiliationId)
        val total = rankingRepository.countRanked(period, affiliationId).toLong()
