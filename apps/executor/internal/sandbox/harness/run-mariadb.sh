...
#
# CSV 로 낸다 (#532). 전에는 탭을 `|` 로 바꿔 냈는데, **값에 `|` 가 있으면 구분자와
# 구분되지 않아** 판정이 틀릴 수 있었다.
#
# 탭으로 가르는 것 자체는 안전하다 — `-B`(batch)가 값에 든 탭·줄바꿈·역슬래시를
# 이스케이프해서 내놓기 때문이다. 그래서 탭으로 가른 뒤 CSV 로 다시 적는다.
#
# **파이프로 잇지 않는다.** `mariadb | awk` 로 두면 스크립트의 종료 코드가 awk 의 것이 되어,
