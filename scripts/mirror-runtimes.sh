...
# 그래서 어느 쪽에서 받아도 같은 것이 온다. (홈랩 zot 에서 실측 확인 — docs/09 §6.2)
#
# **레지스트리가 sync 를 지원하면 이 스크립트는 필요 없다.** zot 은 요청받을 때 업스트림에서
# 직접 당겨올 수 있고(`extensions.sync`), 그쪽이 낫다 — 미는 경로는 앞단의 프록시·CDN 을
# 지나므로 거기서 막힌다. 홈랩에서 실제로 두 곳에서 막혔다:
#   - CDN 을 지나는 경로에서 blob PUT 이 400
#   - 큰 레이어가 리버스 프록시의 기본 read timeout(60초)에 잘려 502
# sync 는 업로드를 하지 않으므로 둘 다 해당하지 않는다.
#
# 그래도 이 스크립트를 남기는 이유: sync 가 없는 레지스트리(하버·ECR 등)에서는 이 방법뿐이다.
#
# 사용:
